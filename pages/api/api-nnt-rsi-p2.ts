...
export default async function handler(
	...
	try {
		if (USE_EXTERNAL_API) {
			// Use external Python API if configured
			const response = await fetch(`${PYTHON_API_URL}/calculate_nnt_rsi_p2`, {
				method: 'POST',
				headers: {
			...
			// Fallback to local Python process (development only)
			const pythonProcess = spawn('python', [
				path.join(process.cwd(), 'strategies', 'nnt-rsi-p2', 'nnt-rsi-p2.py'),
				symbol as string,
				timeframe as string
			]);
